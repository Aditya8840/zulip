...
export let select_top_left_corner_item = function (narrow_to_activate: string): void {
    ...
    if (narrow_to_activate !== "") {
        ...
        const view_match_key = narrow_to_activate.match(/\.top_left_(.+)/);

        let fragment = undefined;
        if (view_match_key && view_match_key[1]) {
...
export function update_sidebar_for_navigation_views(): void {
    ...
    const primary_condensed_views =
        get_built_in_primary_condensed_views();
    const all_built_in_views = get_built_in_views();
    const active_fragment = current_active_fragment;

    const expanded_views = all_built_in_views.filter((view) => {
        return view.is_pinned || view.fragment === active_fragment;
    });
    const active_view =  expanded_views.filter((view) => view.fragment === active_fragment)[0];
    const has_unpinned_views = all_built_in_views.length > expanded_views.length;
